...
async function scanProjectDirectory(projectPath: string) {
  ...
  try {
    ...
    if (pathname === '/api/generate-report') {
      ...
      try {
        ...
        const filesWithDownload = result.files.map((f: { path: string; size: number }) => ({
          ...f,
