...
export interface ProjectData {
  ...
  warnings: string[];
  qc_issues: string[];
  pca_plots?: Array<{ src: string; title: string }>;
  correlation_plots?: Array<{ src: string; title: string }>;
  // 16S Metagenome fields
...
async function getTableDataFromCsv(filePath: string): Promise<TableData | null> {
  try {
...
async function parseMetagenomeData(inputDir: string): Promise<Record<string, any>> {
  // 1. Raw Stats
  let metagenome_raw_stats: any[] = [];
  ...
  // 2. Feature Summary
  let metagenome_feature_summary: any[] = [];
  ...
  // 4. Alpha Diversity
  let metagenome_alpha_diversity: any[] = [];
  ...
  // 6. Image Discovery
  const findPngInDir = async (dirName: string, pattern: RegExp): Promise<string> => {
    ...
    if (await fs.pathExists(dirPath)) {
      ...
      if (matched) return path.join(dirPath, matched);
    }
    return '';
  };
...
export async function parseProjectData(
  ...
  let pca_plots: Array<{ src: string; title: string }> = [];
  let correlation_plots: Array<{ src: string; title: string }> = [];
  ...
  const refLink = wetLabData.ref_genome_link || (metadataOverride?.ref_genome_link as string) || (ref_stats?.ref_genome_link as string) || 'https://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/000/001/405/GCF_000001405.40_GRCh38.p14/';
  if (!static_snippets.reference) {
    (static_snippets as any).reference = {};
  }
  (static_snippets.reference as any).genome_source = `Based on information received from client, the reference genome of ${reference_organism} was downloaded from (${refLink}) and considered for analysis.`;
  ...
  return {
    ...
    project_pi:
      ...
      '',
    submitted_to: wetLabData.submitted_to || (metadataOverride?.submitted_to as string) || 'Dr. Amit Gupta',
...
async function getGODistribution(inputDir: string): Promise<Array<Record<string, unknown>>> {
  ...
  return ensureRowsHaveKeys(
    stats,
    ['combination', 'sig_dge', 'sq_go', 'bp', 'cc', 'mf'],
    'GO statistics unavailable',
    { combination: 'Expected in z2_sigDGE_GO_Statistics.txt' }
  );
}
...
async function extractFunctionalAssets(inputDir: string): Promise<Record<string, unknown>> {
  ...
  let keggPathwayImg: string | null = null;
  if (pathwayImgDir) {
    const pathwayImages: string[] = [];
    async function searchImages(dir: string) {
      const entries = await fs.readdir(dir, { withFileTypes: true });
      for (const entry of entries) {
...
async function extractPcaAndCorrelationAssets(inputDir: string): Promise<{
  pca_plots: Array<{ src: string; title: string }>;
  correlation_plots: Array<{ src: string; title: string }>;
}> {
  const pca_plots: Array<{ src: string; title: string }> = [];
  const correlation_plots: Array<{ src: string; title: string }> = [];
