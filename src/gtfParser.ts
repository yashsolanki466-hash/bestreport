...
function extractAttr(attrs: string, key: string): string {
  ...
  if (gff3Match) return gff3Match[1].replace(/"/g, '');
...
export async function parseGtfStats(gtfPath: string): Promise<GtfStats> {
  const stat = await fs.stat(gtfPath);
  if (stat.size > 5 * 1024 * 1024 * 1024) { // Increased limit to 5GB
    throw new Error(
      `Annotation file too large to scan (${(stat.size / 1e9).toFixed(1)} GB). Set total_genes in metadata.json.`
    );
  }
  ...
  for await (const line of rl) {
    ...
    if (!line || line.startsWith('#')) {
      if (line?.startsWith('#')) {
        ...
        if (orgMatch) organism = orgMatch[1];
      }
      continue;
    }
...
export async function getFastaGenomeStats(fastaPath: string): Promise<{ total: number; length: number; max: number; mean: number }> {
  let total = 0;
  let totalLength = 0;
  let maxLength = 0;
  let currentLength = 0;
...
export async function countFastaSequences(fastaPath: string): Promise<number> {
  let count = 0;
  const rl = createInterface({
    input: createReadStream(fastaPath, { encoding: 'utf8' }),
    crlfDelay: Infinity
...
export async function findReferenceGtf(refDir: string): Promise<string | null> {
  if (!(await fs.pathExists(refDir))) return null;

  const entries = await fs.readdir(refDir);
  const candidates: Array<{ path: string; size: number; score: number }> = [];

  for (const name of entries) {
    if (!/\.(gtf|gff3?|gff)$/i.test(name)) continue;
    const fullPath = path.join(refDir, name);
    const stat = await fs.stat(fullPath);
    if (!stat.isFile()) continue;

    let score = 0;
    if (/\.gtf$/i.test(name)) score += 10;
    if (/gff3/i.test(name)) score += 5;
    if (/merged|backup|raw|whole|genome/i.test(name)) score -= 5;
  ...
  if (chosen.size > MAX_FULL_READ_BYTES) {
    console.warn(
      `Using large annotation file (${(chosen.size / 1e6).toFixed(0)} MB); streaming parser will be used.`
    );
  }
  return chosen.path;
}

export async function findReferenceFasta(refDir: string): Promise<string | null> {
  if (!(await fs.pathExists(refDir))) return null;
  const entries = await fs.readdir(refDir);
  const fastaFiles: Array<{ path: string; size: number }> = [];

  for (const name of entries) {
    if (!/\.(fasta|fa|fna)$/i.test(name)) continue;
    const fullPath = path.join(refDir, name);
    const stat = await fs.stat(fullPath);
    if (stat.isFile()) fastaFiles.push({ path: fullPath, size: stat.size });
  }

  if (fastaFiles.length === 0) return null;
