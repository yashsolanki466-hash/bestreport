...
export class ReportGenerator {
  ...
  private async generatePDF(html: string, outputPath: string, basePath: string): Promise<void> {
    ...
    try {
      ...
      await page.pdf({
        ...
        footerTemplate:
          '<div style="font-size:9px;text-align:center;width:100%;color:#666;font-family:Georgia,serif;">Page <span class="pageNumber"></span> of <span class="totalPages"></span></div>'
      });
    } finally {
      await page.close();
  ...
  private async generateDOCX(data: ProjectData, outputPath: string, templateName?: string): Promise<void> {
    const children: any[] = [];
    const wl = (data.static_snippets as any)?.wet_lab || {};
    ...
    const addImageIfExist = (imageSrc?: string, width = 450, height = 300) => {
      if (!imageSrc) return;
    ...
    if (templateName === 'report_interim') {
      ...
      const isDna = !!(data.service_type && (
        data.service_type.toLowerCase().includes('16s') || 
      ...
      const tocTableRows: TableRow[] = tocItems.map((item) => {
        return new TableRow({
          children: [
            new TableCell({
...
function getImageBufferAndType(imageSrc: string): { buffer: Buffer; extension: string } | null {
...
function createHeading(text: string, level: any, headingNum?: string): Paragraph {
...
function createCalloutBox(text: string, title?: string): Table {
  return new Table({
    width: { size: 100, type: WidthType.PERCENTAGE },
    borders: {
      top: { style: BorderStyle.NONE },
...
function createParagraph(text: string, options: { bold?: boolean; italic?: boolean; color?: string; spacingAfter?: number } = {}): Paragraph {
  return new Paragraph({
    spacing: { 
...
function createCoverPage(data: ProjectData, templateName?: string): (Paragraph | Table)[] {
