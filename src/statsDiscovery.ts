...
export async function resolvePathCaseInsensitive(baseDir: string, relPath: string): Promise<string | null> {
  const parts = relPath.split(/[/\\]/);
  let current = baseDir;
  for (const part of parts) {
    if (!part) continue;
    try {
      if (!(await fs.pathExists(current))) return null;
...
export async function findStatsWorkbook(
  inputDir: string,
  matchers: RegExp[],
  searchDirs: string[]
): Promise<string | null> {
