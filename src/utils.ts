...
export async function validateProjectDirectory(inputDir: string): Promise<{ valid: boolean; error?: string }> {
  try {
    ...
    const hasExpectedStructure = expectedFolders.some(folder => 
      contents.includes(folder)
    );
...
export async function findProjects(rootDir: string, maxDepth: number = 2, pattern: string = '*'): Promise<string[]> {
...
async function checkProjectStructure(dir: string): Promise<boolean> {
  try {
